...
impl GpuScene {
    ...
    pub fn upload_data(&mut self, device: &Arc<Device>) {
        ...
        for (key, mesh_buf) in self.meshes_bufs.iter() {
        ...
        for instance in self.instances.values() {
            ...
            instancedata.push(GlslInstanceData {
                ...
                indices: self.meshes_bufs[instance.mesh].index,
                vertices: self.meshes_bufs[instance.mesh].index,
                normal_uv_mask: 0,
            });
        }
    ...
    pub fn insert_mesh(
        ...
        let mesh = self.meshes_bufs.insert(GpuIndexed {
            val: (
    ...
    pub fn append_gltf(&mut self, device: &Arc<Device>) {
        ...
        let (gltf, buffers, _) = gltf::import(path).unwrap();
        {
            ...
            for mesh in gltf.meshes() {
                ...
                let entity = self.insert_mesh(device, &indices.0, &vertices.vertices);
